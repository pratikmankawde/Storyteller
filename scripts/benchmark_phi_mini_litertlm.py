#!/usr/bin/env python3
"""
Benchmark script for Phi-mini (LiteRT-LM) model.
...
Model: phi-mini (LiteRT-LM format)
...
MODEL_PATH = os.environ.get(
    "PHI_MINI_MODEL_PATH",
    r"D:\Learning\Ai\Models\LLM\Phi\Phi-4-mini-instruct_multi-prefill-seq_q8_ekv4096.litertlm"
)
...
# Must match the registry alias exactly for lit CLI to find it
LIT_MODEL_ALIAS = "phi-4-mini"
LIT_MODEL_FILENAME = "Phi-4-mini-instruct_multi-prefill-seq_q8_ekv4096.litertlm"

# Token-based input sizing (model context 4069; ~4 chars/token for English)
CHARS_PER_TOKEN = 4
...
def start_lit_session(model_path: str, backend: str) -> LitSession | None:
    ...
    if Path(model_path).is_file():
        # lit CLI looks for models in user's home directory with exact registry filename
        cache_dir = Path.home() / ".litert-lm" / "models"
        cache_dir.mkdir(parents=True, exist_ok=True)
        cached = cache_dir / LIT_MODEL_FILENAME
        src_size = Path(model_path).stat().st_size
        if not cached.is_file() or cached.stat().st_size != src_size:
            import shutil
            print(f"Copying model to lit cache: {cached}")
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Phi-mini LiteRT-LM benchmark")
    ...
    print("=" * 60)
    print("Phi-mini (LiteRT-LM) benchmark")
    print("=" * 60)

    if not Path(MODEL_PATH).exists():
        print(f"ERROR: Model not found at {MODEL_PATH}")
        print("Set PHI_MINI_MODEL_PATH to your phi-mini LiteRT-LM model path.")
        sys.exit(1)
    ...
    if out_path:
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            f.write("Phi-mini LiteRT-LM benchmark (per chapter)\n")
            f.write("=" * 60 + "\n")
    ...
    out_file = out_path or (OUTPUT_DIR / "phi_mini_litertlm_benchmark_output.txt")
    if not out_file.is_absolute():
        out_file = OUTPUT_DIR / out_file
    with open(out_file, "w", encoding="utf-8") as f:
        f.write("Phi-mini LiteRT-LM benchmark\n")
        f.write("=" * 60 + "\n")
        f.write(f"Model: {MODEL_PATH}\n")
