...
def get_dit_example_input_mapping(dtype=torch.float):
    ...
    return {
        "x": torch.rand(size=(1, 64, 256), dtype=dtype, requires_grad=False),
        "t": torch.tensor([0.154], dtype=dtype, requires_grad=False),
        "cross_attn_cond": torch.rand(size=(1, 65, 768), dtype=dtype, requires_grad=False),
        "global_cond": torch.rand(size=(1, 768), dtype=dtype, requires_grad=False),
    }
...
class AutoEncoderEncoderModule(torch.nn.Module):
    """Wrap the AutoEncoder Encoder Module."""
...
def export_audiogen(args) -> None:
    ...
    logging.info("Exporting the model, ckpt: %s, with config %s.", args.ckpt_path, args.model_config)
    ...
    # Workaround for LiteRT runtime issue
    rotary_pos_emb_res = dit_model.model.transformer.rotary_pos_emb.forward_from_seq_len(257)
    ...
    edge_model = ai_edge_torch.convert(autoencoder_decoder, autoencoder_decoder_example_input)
    edge_model.export("./autoencoder_model.tflite")
    logging.info("AutoEncoder Decoder model has been saved to ./autoencoder_model.tflite")

    ## --------- AutoEncoder Encoder Model (optional, for audio-to-audio) ---------
    ...
    edge_model = ai_edge_torch.convert(autoencoder_encoder, autoencoder_encoder_example_input)
    edge_model.export("./autoencoder_encoder_model.tflite")
    logging.info("AutoEncoder Encoder model has been saved to ./autoencoder_encoder_model.tflite")

    logging.info("All models have been exported successfully!")


def main():
    """Main function to export the AudioGen DiT and AutoEncoder models to LiteRT format."""
    parser = argparse.ArgumentParser()
    parser.add_argument("-m", "--model_config", type=str, help="Path to the model configuration file.", required=True)
    parser.add_argument("-p", "--ckpt_path", type=str, help="Path to the model checkpoint file.", required=True)
