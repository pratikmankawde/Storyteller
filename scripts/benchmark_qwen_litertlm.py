#!/usr/bin/env python3
"""
Benchmark script for Qwen 2.5 1.5B (LiteRT-LM) model.
...
Model: Qwen2.5-1.5B-Instruct (LiteRT-LM format)
...
MODEL_PATH = os.environ.get(
    "QWEN_MODEL_PATH",
    r"D:\Learning\Ai\Models\LLM\Qwen\Qwen2.5-1.5B-Instruct_multi-prefill-seq_q8_ekv4096.litertlm"
)
...
LIT_MODEL_ALIAS = "qwen2.5-1.5b"
LIT_MODEL_FILENAME = "Qwen2.5-1.5B-Instruct_multi-prefill-seq_q8_ekv4096.litertlm"
...
def main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Qwen 2.5 1.5B LiteRT-LM benchmark")
    ...
    print("=" * 60)
    print("Qwen 2.5 1.5B (LiteRT-LM) benchmark")
    print("=" * 60)

    if not Path(MODEL_PATH).exists():
        print(f"ERROR: Model not found at {MODEL_PATH}")
        print("Set QWEN_MODEL_PATH to your Qwen LiteRT-LM model path.")
        sys.exit(1)
    ...
    if out_path:
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            f.write("Qwen 2.5 1.5B LiteRT-LM benchmark (per chapter)\n")
            f.write("=" * 60 + "\n")
    ...
    out_file = out_path or (OUTPUT_DIR / "qwen_litertlm_benchmark_output.txt")
    if not out_file.is_absolute():
        out_file = OUTPUT_DIR / out_file
    with open(out_file, "w", encoding="utf-8") as f:
        f.write("Qwen 2.5 1.5B LiteRT-LM benchmark\n")
        f.write("=" * 60 + "\n")
        f.write(f"Model: {MODEL_PATH}\n")
