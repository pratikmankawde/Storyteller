...
class ReaderFragment : Fragment(), PlayerBottomSheet.PlayerControlsListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        novelPageAdapter = NovelPageAdapter(highlightColor) { pageIndex ->
        ...
        playbackEngine = PlaybackEngine(requireContext(), app.ttsEngine).apply {
            ...
            setOnCompleteListener {
                ...
                btnPlay?.text = "Play"
                if (isNovelFormat) novelPageAdapter?.clearHighlighting()
                // my_prompt: when reading of chapter is finished, perform same analysis for next chapter
                viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    if (idx >= 0 && idx + 1 < chapters.size) {
                        val nextCh = chapters[idx + 1]
                        if (nextCh.fullAnalysisJson.isNullOrBlank() && nextCh.body.length > 50) {
                            AppLogger.d(tag, "Analyzing next chapter (on reach): ${nextCh.title}")
                            // One LLM call only; skip extraction and extended analysis to avoid 2+ min delay
                            val resp = QwenStub.analyzeChapter(nextCh.body)
                            if (resp != null) {
                                app.bookRepository.updateChapter(nextCh.copy(
                                ...
                                resp.characters?.let { chars ->
                                    context?.let { ctx ->
                                        MergeCharactersUseCase(app.db.characterDao(), ctx)
                                            .mergeAndSave(bookId, listOf(gson.toJson(chars)))
                                    }
                                }
                                AppLogger.i(tag, "Next chapter analysis done: ${nextCh.title}")
                            }
                        }
                    }
                }
        ...
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            ...
            // Lazy chapter analysis: one LLM call only; extended analysis deferred to Insights tab
            if (ch != null && ch.fullAnalysisJson.isNullOrBlank() && chapterText.length > 50) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(requireContext(), "Analyzing chapter…", Toast.LENGTH_SHORT).show()
                }
                val resp = QwenStub.analyzeChapter(chapterText)
                if (resp != null) {
                    ch = ch.copy(
                        summaryJson = gson.toJson(resp.chapterSummary),
                        analysisJson = null,
                    ...
                    app.bookRepository.updateChapter(ch)
                    chapterAnalysis = resp
                    resp.characters?.let { chars ->
                        MergeCharactersUseCase(app.db.characterDao(), requireContext())
                            .mergeAndSave(bookId, listOf(gson.toJson(chars)))
                    }
                    AppLogger.i(tag, "Lazy analysis done: dialogs=${resp.dialogs?.size ?: 0}, characters=${resp.characters?.size ?: 0}")
                }
            }

            // Parse analysis from DB if we didn't just run lazy analysis
            ...
            withContext(Dispatchers.Main) {
                if (isNovelFormat) {
                    ...
                    AppLogger.i(tag, "Split chapter (${chapterText.length} chars) into ${novelPages.size} pages")
                    ...
                    // Load any saved page audio and pre-generate missing first 2 pages
                    if (novelPages.isNotEmpty()) {
                        loadSavedPageAudio()
                        preGenerateAudioForPages(0, minOf(2, novelPages.size))
                    }
                } else {
        ...
        view.findViewById<View>(R.id.recap_listen)?.setOnClickListener {
            viewLifecycleOwner.lifecycleScope.launch {
                ...
                result.onSuccess { audioFile ->
                    ...
                    android.util.Log.e("ReaderFragment", "TTS synthesis failed", error)
                    Toast.makeText(requireContext(), "TTS failed: ${error.message}", Toast.LENGTH_SHORT).show()
                }
            }
        }
            ...
            viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
                ...
                val result = runCatching {
                    app.db.bookmarkDao().insert(
                        Bookmark(bookId = bookId, chapterId = idToSave, paragraphIndex = 0, contextSummary = summary)
                    )
                }
                withContext(Dispatchers.Main) {
                    ...
                    if (result.isSuccess) {
                        ...
                        Toast.makeText(requireContext(), "Failed to save bookmark", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
    ...
    override fun onRewind() {
        val currentPos = playbackEngine?.getCurrentPosition() ?: 0L
    ...
    override fun onForward() {
        val currentPos = playbackEngine?.getCurrentPosition() ?: 0L
        val duration = playbackEngine?.getDuration() ?: 0L
    ...
    /**
     * Load saved page audio for this book/chapter into preGeneratedAudio so it can be reused.
     */
    private fun loadSavedPageAudio() {
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                AppLogger.e(tag, "Error loading saved page audio", e)
            }
        }
    }
     ...
     * Uses saved audio if available; otherwise generates via TTS and saves for reuse.
     * - On chapter load: first 2 pages are pre-generated (after loadSavedPageAudio).
        ...
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            try {
                for (i in startPage until minOf(startPage + count, novelPages.size)) {
                    ...
                    AppLogger.d(tag, "Pre-generating audio for page $i in background (${pageText.length} chars)")
                    ...
                    result.onSuccess { audioFile ->
                        audioFile?.let { file ->
                            ...
                            AppLogger.i(tag, "Pre-generated and saved audio for page $i: ${persisted.absolutePath}")
                        }
                    }.onFailure { error ->
                        AppLogger.e(tag, "Failed to pre-generate audio for page $i", error)
                    }
                }
            } catch (e: Exception) {
            ...
            withContext(Dispatchers.IO) {
                ...
                result.onSuccess { audioFile ->
                    audioFile?.let { file ->
                        ...
                        withContext(Dispatchers.Main) {
                            btnPlay?.isEnabled = true
                            Toast.makeText(requireContext(), "Failed to generate audio", Toast.LENGTH_SHORT).show()
                        }
                    }
                }.onFailure { error ->
                    withContext(Dispatchers.Main) {
                        btnPlay?.isEnabled = true
                        Toast.makeText(requireContext(), "TTS error: ${error.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
