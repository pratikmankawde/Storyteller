...
|------|--------|---------------|---------------------------|
| **Qwen2.5-3B-Instruct (4-bit GGUF)** | Chapter analysis (characters, dialogs, summaries, sound cues) | Hugging Face / llama.cpp releases | Device **Downloads** folder (app looks for `qwen2.5-3b-instruct-q4_k_m.gguf` there) |
...
|------|--------|-------------|
| **Model paths** | Where Qwen GGUF, SherpaTTS assets, and (if any) SFX model live on device | When moving from stubs to real inference |
...
- **LLM (Qwen)**  
  ...
  - You need: GGUF path, llama.cpp-style Android lib, and JNI/bridge code.
...
2. **Add real models**  
   - Drop Qwen GGUF (and Android inference lib) and point the LLM stub to them.  
